#  Copyright (C) 2022-2026 Trace Developers
#
...
param(
    ...
    [string]$SymbolStoreProduct = "trace",
    
    [Parameter(Mandatory=$False, ParameterSetName="publish_symstore")]
    [Switch]$CleanOldSymbols,
    
    [Parameter(Mandatory=$False, ParameterSetName="publish_sentry")]
    [string]$SentryOrg = "trace",
    
    [Parameter(Mandatory=$False, ParameterSetName="publish_sentry")]
    [string]$SentryProject = "trace"
)
...
if( $PublishSentry ) {
    if( (Get-Item $SourcePath) -is [System.IO.DirectoryInfo] ) {
        ...
        sentry-cli releases -o $SentryOrg -p $SentryProject set-commits --commit "Trace / Trace Source Code / trace@$commitHash" $commitHash
    } else {
