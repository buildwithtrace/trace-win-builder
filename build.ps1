#  Copyright (C) 2021-2026 Trace Developers
#
...
##
# Trace Powershell Windows Build Assistant
#
...
param(
    ...
    [Parameter(Mandatory=$False, ParameterSetName="package")]
    [Parameter(Mandatory=$False, ParameterSetName="preparepackage")]
    [string]$BuildConfigName = 'trace-nightly',
...
function Get-Source-Path([string]$subfolder) {
    # Special handling for local Trace source
    if ($subfolder -eq "trace" -and $buildConfig.sources.trace.ref -eq "local") {
        # Return parent directory + Trace subfolder where Trace source lives
        $parentDir = Split-Path -Parent $PSScriptRoot
        return Join-Path -Path $parentDir -ChildPath "Trace"
    }
...
function Start-Build {
    ...
    # Only checkout if not using local source
    if ($buildConfig.sources.trace.ref -ne "local") {
        Get-Source -url (Get-Source-Repo -sourceKey "trace" -defaultRepo "https://gitlab.com/trace/code/trace.git") `
                   -dest (Get-Source-Path trace) `
                   -sourceType git `
                   -latest $latest `
                   -ref (Get-Source-Ref -sourceKey "trace")
    } else {
        Write-Host "Using local Trace source at $(Get-Source-Path trace)" -ForegroundColor Cyan
    }

    # NOTE: Library downloads are now handled by FetchContent in Trace's CMakeLists.txt
    # FetchContent automatically downloads kicad-symbols, kicad-footprints, and kicad-packages3D
    # during cmake configure and installs them to share/trace/ during cmake --install.
    # The separate library builds are no longer needed.

    Build-Trace -arch $arch -buildType $buildType
}
...
function Get-Trace-Version {
    $srcFile = Join-Path -Path (Get-Source-Path trace) -ChildPath "CMakeModules\TraceVersion.cmake"
    if( -not (Test-Path $srcFile ) )
    {
        # new path in master
        $srcFile = Join-Path -Path (Get-Source-Path trace) -ChildPath "cmake\TraceVersion.cmake"
    }


    $result = Select-String -Path $srcFile -Pattern '(?<=TRACE_SEMANTIC_VERSION\s")([0-9]+).([0-9])+' -AllMatches | % { $_.Matches } | % { $_.Value } | Select-Object -First 1

    return $result
}

function Get-Trace-PackageVersion-Msix {

    $base = Get-Trace-Version
...
function Start-Prepare-Package {
    ...
    Get-Trace-CommitHash | Out-File -FilePath $commitHashPath -Encoding ascii

    $packageVersion = Get-Trace-PackageVersion
    $traceVersion = Get-Trace-Version

    $nsisArch = Get-NSISArch -Arch $arch

    Write-Host "Package Version: $packageVersion"
    Write-Host "Trace Version: $traceVersion"
    if($lite) {
    ...
    if($buildConfig.vcpkg.manifest_mode) {
        $tracePath = Get-Source-Path trace
        $vcpkgInstalledRoot = Join-Path -Path $tracePath -ChildPath "build/$buildName/vcpkg_installed/$triplet/"
    } else {
    ...
    Install-Trace -arch $arch -buildType $buildType
    ...
    # NOTE: Library installation is now handled by FetchContent in Trace's CMakeLists.txt
    # The Install-Trace call above runs cmake --install which installs:
    # - symbols to share/trace/symbols
    # - footprints to share/trace/footprints  
    # - 3dmodels to share/trace/3dmodels
    # - templates to share/trace/template
    # The separate kicad-* library repos are no longer needed for packaging.
    ...
    $traceCmdSource = Join-Path -Path $PSScriptRoot -ChildPath "\support\trace-cmd.bat"
    Copy-Item $traceCmdSource -Destination $destBin -Force
...
function Start-Package-Nsis {
    ...
    $badFile = Join-Path -Path $destRoot -ChildPath "share\trace\demos\kit-dev-coldfire-xilinx_5213\kit-dev-coldfire.pretty\DSUB-9_Female_Horizontal_P2.77x2.84mm_EdgePinOffset7.70mm_Housed_MountingHolesOffset9.12mm.kicad_mod"
    ...
    $readmeSrc = Join-Path -Path (Get-Source-Path trace) -ChildPath "LICENSE.README"
    ...
    $destTraceShare = Join-Path -Path $destRoot -ChildPath "share\trace"

    if( $lite )
    {
        # needed for lite mode to enable footprints and symbols, why? who knows for now
        New-Item -ItemType "directory" -Path (Join-Path -Path $destTraceShare -ChildPath "\footprints")
        New-Item -ItemType "directory" -Path (Join-Path -Path $destTraceShare -ChildPath "\symbols")
...
function Create-AppxManifest {
    ...
    $manifest = $manifest.replace("[TRACE_VERSION]", $TraceVersion)
...
function Start-Package-Msix {
    ...
    $packageVersion = Get-Trace-PackageVersion-Msix
    $traceVersion = Get-Trace-Version
    ...
    $destRootVfs = Join-Path -Path $destRoot -ChildPath "\VFS\ProgramFilesX64\Trace\5.99\"
...
function Generate-Msix-Assets {
    ...
    $traceStoreIconSource = Join-Path -Path $iconSources -ChildPath "icon_trace.svg"
    $traceStoreIconDest = Join-Path -Path $iconSources -ChildPath "icon_trace_store.svg"
    Convert-Svg -Svg $traceStoreIconSource -Width 300 -Height 300 -Out "$iconDest/icon_trace_store.png"
