...
!macro _DeleteFileAssociationFunc EXT
  ;be sure to only delete the specific trace.extension.version entry
  DeleteRegValue ${SOFTWARE_CLASSES_ROOT_KEY} "Software\Classes\.${EXT}\OpenWithProgids\" "${FILE_ASSOC_PREFIX}.${EXT}.${TRACE_VERSION}"
  ;delete the entire trace.extension.version key set
  DeleteRegKey ${SOFTWARE_CLASSES_ROOT_KEY} "Software\Classes\${FILE_ASSOC_PREFIX}.${EXT}.${TRACE_VERSION}"
!macroend
...
!macro _CreateFileAssociationFunc
  ...
  WriteRegExpandStr ${SOFTWARE_CLASSES_ROOT_KEY} "Software\Classes\.$R0\OpenWithProgids\" "${FILE_ASSOC_PREFIX}.$R0.${TRACE_VERSION}" ""

  ;program level extension entry
  WriteRegExpandStr ${SOFTWARE_CLASSES_ROOT_KEY} "Software\Classes\${FILE_ASSOC_PREFIX}.$R0.${TRACE_VERSION}" "" "$R2"
  WriteRegExpandStr ${SOFTWARE_CLASSES_ROOT_KEY} "Software\Classes\${FILE_ASSOC_PREFIX}.$R0.${TRACE_VERSION}\DefaultIcon" "" "$INSTDIR\bin\$R1,$R3"
  WriteRegExpandStr ${SOFTWARE_CLASSES_ROOT_KEY} "Software\Classes\${FILE_ASSOC_PREFIX}.$R0.${TRACE_VERSION}\shell\open\command" "" '$INSTDIR\bin\$R1 "%1"'
...
!macro _RegisterApplicationFunc
  ...
  ; Windows would then use this friendly name and disable it over all versions of Trace in the open with
