...
; Copyright (C) 2020-2026 Trace Team
;
...
; (Trace/NSIS by default). When the build is updated the product and installer
...
!define TRACE_MULTIUSER_INSTALLMODE_64_BIT_32BITVIEW 1 ; We added this hack because we had 32-bit regview by default in 7.0 and are adding multi-user midcycle
...
!define FILE_ASSOC_PREFIX	"Trace"
...
!ifndef OUTFILE
  !define OUTFILE "trace-${PACKAGE_VERSION}-${OPTION_STRING}.exe"
!endif
OutFile ${OUTFILE}
...
BrandingText "Trace installer for Windows"
...
!define MUI_HEADERIMAGE_BITMAP "trace-header.bmp" ; optional
!define MUI_WELCOMEFINISHPAGE_BITMAP "trace-welcome.bmp"
...
VIAddVersionKey "FileDescription" "Installer for the Trace AI-Powered PCB Design Suite"
...
!macro TraceRunningProccessesCheck
  ${RunningProcessCheck} "trace.exe" $(APP_NAME_TRACE)
...
Function onDirectoryPageLeave
  !insertmacro TraceRunningProccessesCheck
...
Section $(TITLE_SEC_MAIN) SEC01
  ...
  RMDir /r "$INSTDIR\share\trace\library"
  RMDir /r "$INSTDIR\share\trace\modules"
  ...
  SetOutPath "$INSTDIR\share\trace\internat"
  File /nonfatal /r "..\share\trace\internat\*"

  SetOutPath "$INSTDIR\share\trace\template"
  File /nonfatal /r "..\share\trace\template\*"

  SetOutPath "$INSTDIR\share\trace\resources"
  File /nonfatal /r "..\share\trace\resources\*"

  SetOutPath "$INSTDIR\share\trace\schemas"
  File /nonfatal /r "..\share\trace\schemas\*"
...
!endif
  ...
  SetOutPath "$INSTDIR\share\trace\scripting\kicad_pyshell"
  File /nonfatal /r "..\share\trace\scripting\kicad_pyshell\*"

  SetOutPath "$INSTDIR\share\trace\scripting\plugins"
  File /nonfatal /r "..\share\trace\scripting\plugins\*"

  SetOutPath "$INSTDIR\share\trace\scripting\trace"
  File /nonfatal /r "..\share\trace\scripting\trace\*"
...
SectionGroup /e $(TITLE_SEC_LIBRARIES) SEC03
  ...
  Section $(TITLE_SEC_SCHLIB) SEC03_SCHLIB
    ...
    SetOutPath "$INSTDIR\share\trace\symbols"
    File /nonfatal /r "..\share\trace\symbols\*"

    !insertmacro RecursiveReadOnlyFlagFiles "$INSTDIR\share\trace\symbols\"
  SectionEnd
  !else
  Section /o $(TITLE_SEC_SCHLIB) SEC03_SCHLIB
    AddSize 24576 ; 24MB
    !insertmacro DownloadAndExtract "${KICAD_SYMBOLS_FILE}" "${KICAD_SYMBOLS_URL}" "symbols" "${KICAD_SYMBOLS_FOLDER}" "$INSTDIR\share\trace\" "symbols"
  ...
  Section $(TITLE_SEC_FPLIB) SEC03_FOOTPRINTS
    ...
    SetOutPath "$INSTDIR\share\trace\footprints"
    File /nonfatal /r "..\share\trace\footprints\*"

    !insertmacro RecursiveReadOnlyFlagFiles "$INSTDIR\share\trace\footprints\"
  SectionEnd
  !else
  Section /o $(TITLE_SEC_FPLIB) SEC03_FOOTPRINTS
    AddSize 81920 ; 80MB
    !insertmacro DownloadAndExtract "${KICAD_FOOTPRINTS_FILE}" "${KICAD_FOOTPRINTS_URL}" "footprints" "${KICAD_FOOTPRINTS_FOLDER}" "$INSTDIR\share\trace\" "footprints"
  ...
  Section $(TITLE_SEC_PACKAGES3D) SEC03_PACKAGES3D
    ...
    SetOutPath "$INSTDIR\share\trace\3dmodels"
    File /nonfatal /r "..\share\trace\3dmodels\*"

    !insertmacro RecursiveReadOnlyFlagFiles "$INSTDIR\share\trace\3dmodels\"
  SectionEnd
  !else
  Section /o $(TITLE_SEC_PACKAGES3D) SEC03_PACKAGES3D
    AddSize 5767168 ; 5.5GB
    !insertmacro DownloadAndExtract "${KICAD_PACKAGES3D_FILE}" "${KICAD_PACKAGES3D_URL}" "3d models" "${KICAD_PACKAGES3D_FOLDER}" "$INSTDIR\share\trace\" "3dmodels"
...
Section $(TITLE_SEC_DEMOS) SEC05
  ...
  SetOutPath "$INSTDIR\share\trace\demos"
  File /nonfatal /r "..\share\trace\demos\*"
  SetOutPath "$INSTDIR\share\doc\trace\tutorials"
  File /nonfatal /r "..\share\doc\trace\tutorials\*"
SectionEnd
...
SectionGroup $(TITLE_SEC_DOCS) SEC06
  Section $(LANGUAGE_NAME_EN) SEC06_EN
    ...
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\en"
    File /nonfatal /r "..\share\doc\trace\help\en\*"
  SectionEnd
  Section $(LANGUAGE_NAME_DE) SEC06_DE
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\de"
    File /nonfatal /r "..\share\doc\trace\help\de\*"
  SectionEnd
  Section $(LANGUAGE_NAME_ES) SEC06_ES
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\es"
    File /nonfatal /r "..\share\doc\trace\help\es\*"
  SectionEnd
  Section $(LANGUAGE_NAME_FR) SEC06_FR
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\fr"
    File /nonfatal /r "..\share\doc\trace\help\fr\*"
  SectionEnd
  Section $(LANGUAGE_NAME_IT) SEC06_IT
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\it"
    File /nonfatal /r "..\share\doc\trace\help\it\*"
  SectionEnd
  Section $(LANGUAGE_NAME_JA) SEC06_JA
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\ja"
    File /nonfatal /r "..\share\doc\trace\help\ja\*"
  SectionEnd
  Section $(LANGUAGE_NAME_NL) SEC06_NL
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\nl"
    File /nonfatal /r "..\share\doc\trace\help\nl\*"
  SectionEnd
  Section $(LANGUAGE_NAME_PL) SEC06_PL
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\pl"
    File /nonfatal /r "..\share\doc\trace\help\pl\*"
  SectionEnd
  Section $(LANGUAGE_NAME_ZH) SEC06_ZH
	!insertmacro ExclusiveDetailPrint $(INSTALLING_DOCUMENTATION)
    SetOverwrite try
    SetOutPath "$INSTDIR\share\doc\trace\help\zh"
    File /nonfatal /r "..\share\doc\trace\help\zh\*"
  SectionEnd
SectionGroupEnd

Section $(TITLE_SEC_FILE_ASSOC) SEC07
  !insertmacro ExclusiveDetailPrint $(SETTING_FILE_ASSOCS)
  ${CreateFileAssociationNoIcon} "kicad_pcb" "pcbnew.exe" "$(FILE_DESC_KICAD_PCB) ${TRACE_VERSION}"
  ${CreateFileAssociationNoIcon} "sch" "eeschema.exe" "$(FILE_DESC_SCH) ${TRACE_VERSION}"
  ${CreateFileAssociationNoIcon} "kicad_sch" "eeschema.exe" "$(FILE_DESC_SCH) ${TRACE_VERSION}"
  ${CreateFileAssociationNoIcon} "pro" "trace.exe" "$(FILE_DESC_PRO) ${TRACE_VERSION}"
  ${CreateFileAssociationNoIcon} "kicad_pro" "trace.exe" "$(FILE_DESC_PRO) ${TRACE_VERSION}"
  ${CreateFileAssociationNoIcon} "kicad_wks" "pl_editor.exe" "$(FILE_DESC_KICAD_WKS) ${TRACE_VERSION}"
...
Section -CreateShortcuts
  ...
  CreateShortCut "${SMPATH}\Schematic Editor ${TRACE_VERSION}.lnk" "$INSTDIR\bin\eeschema.exe"
  CreateShortCut "${SMPATH}\PCB Editor ${TRACE_VERSION}.lnk" "$INSTDIR\bin\pcbnew.exe"
  CreateShortCut "${SMPATH}\Gerber Viewer ${TRACE_VERSION}.lnk" "$INSTDIR\bin\gerbview.exe"
  CreateShortCut "${SMPATH}\Image Converter ${TRACE_VERSION}.lnk" "$INSTDIR\bin\bitmap2component.exe"
  CreateShortCut "${SMPATH}\Calculator Tools ${TRACE_VERSION}.lnk" "$INSTDIR\bin\pcb_calculator.exe"
  CreateShortCut "${SMPATH}\Drawing Sheet Editor ${TRACE_VERSION}.lnk" "$INSTDIR\bin\pl_editor.exe"
  CreateShortCut "${SMPATH}\Trace ${TRACE_VERSION} Command Prompt.lnk" "%comspec%" '/k "$INSTDIR\bin\trace-cmd.bat"'
...
Section Uninstall
  ...
  RMDir /r "$INSTDIR\share\trace\symbols"
  RMDir /r "$INSTDIR\share\trace\footprints"
  RMDir /r "$INSTDIR\share\trace\3dmodels"
  RMDir /r "$INSTDIR\share\trace\template"
  RMDir /r "$INSTDIR\share\trace\internat"
  RMDir /r "$INSTDIR\share\trace\demos"
  RMDir /r "$INSTDIR\share\trace\scripting\kicad_pyshell"
  RMDir /r "$INSTDIR\share\trace\scripting\plugins"
  RMDir /r "$INSTDIR\share\trace\scripting\trace"
  RMDir /r "$INSTDIR\share\trace\scripting\python_packages"
  RMDir /r "$INSTDIR\share\trace\scripting"

  !insertmacro ExclusiveDetailPrint $(REMOVING_DOCS)
  RMDir /r "$INSTDIR\share\doc\trace\tutorials"
  RMDir /r "$INSTDIR\share\doc\trace\help"
  RMDir /r "$INSTDIR\share\doc\trace"
  ...
  FinishUninstall:
  ;Note - application registry keys are stored in the users individual registry hive (HKCU\Software\trace".
...
Function EnableLiteMode
  ...
  !insertmacro CompileTimeIfFileExist "..\share\trace\symbols" ADD_LIBS
  !ifndef ADD_LIBS
    !insertmacro SetSectionFlag ${SEC03_SCHLIB} ${SF_RO}
    !insertmacro UnselectSection ${SEC03_SCHLIB}
  !endif

  !insertmacro CompileTimeIfFileExist "..\share\trace\footprints" ADD_MODULES
  !ifndef ADD_MODULES
    !insertmacro SetSectionFlag ${SEC03_FOOTPRINTS} ${SF_RO}
    !insertmacro UnselectSection ${SEC03_FOOTPRINTS}
  !endif

  !insertmacro CompileTimeIfFileExist "..\share\doc\trace\help" ADD_HELP
  !ifndef ADD_HELP
