#
# This program source code file is part of Trace, an AI-Powered PCB Design application.
#
...
trace_path = os.path.normpath(os.path.join(current_directory, '..\\..\\trace.exe'))

trace_version = ""
trace_product_version = ""
try:
    trace_product_version = get_file_version(trace_path)
except:
    pass

if trace_product_version:
    version_bits = trace_product_version.split('.')
    if len(version_bits) >= 2:
        trace_version = "{}.{}".format(version_bits[0],version_bits[1])

user_docus_path = get_user_docu_path()

if trace_version and not in_venv:
    # sysconfig override
    user_base = os.path.join(user_docus_path, f'Trace\\{trace_version}\\3rdparty\\')
    ...
    user_site = os.path.join(user_docus_path, f'Trace\\{trace_version}\\3rdparty\\Python{python_ver_nodot}\\site-packages')
